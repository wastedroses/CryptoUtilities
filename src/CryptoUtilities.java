...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //Calls method recursively. finds greatest common denominator
        //excluding 0 and restores n.
        if (!m.isZero()) {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        boolean even = false;
        int remainder = 0;
        //remainder is what is left after n / 10.
        remainder = n.divideBy10();
        //if remainder mod 2 is 0, means it's even.
        if (remainder % 2 == 0) {
            even = true;
        }
        //restores n.
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //if p is not 0 and is even, then it's going to run powerMod function
        //and then copy the value to n2. then n will multiply by itself.
        if (!p.isZero()) {
            if (isEven(p)) {
                p.divide(two);
                powerMod(n, p, m);
                ...
                //same rules here except p isn't even and must be made even
                //through the powerMod function.
                p.decrement();
                powerMod(n, p, m);
                n2.multiply(n);
            }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //powerMod function in order to see if the number is prime or not
        powerMod(w, two, n);
        powerMod(w2, n2, n);

        //if w is one (w being a number near n/witness) and w2 isn't, then
        //this is true.
        if (w.equals(one) || !w2.equals(one)) {
            compositeness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        //subtracting n from 4 to make sure that number is near n.
        n2.subtract(four);
        for (int i = 0; i < 26; i++) {

            //generating random numbers
            random = randomNumber(n2);
            //adding two to random number for the possibility of being prime
            random.add(two);
            ...
            //if n is even, it's not prime.
                ...
                //if isWitnessToCompositeness is false, then isPrime is true.
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        NaturalNumber n2 = new NaturalNumber2(n);
        NaturalNumber two = new NaturalNumber2(2);
        //while loop for isPrime2, and it runs if it's false.
        while (!isPrime2(n2)) {
            //if the number is even, n is incremented to see if the odd number
            //is prime.
            if (isEven(n2)) {
                n2.increment();
            } else {
                //if the number is odd, then it adds 2 to check for the next
                //odd/prime number.
                n2.add(two);
            }
        }
